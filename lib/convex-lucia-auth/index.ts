...
export type DatabaseUserAttributes = Omit<DocumentByName<ConvexLuciaAuth.DataModel, 'users'>, 'id'>
...
export async function getValidSessionAndRenew(auth: Auth, sessionId: string | null) {
	if (sessionId === null) {
		return null
	}
	try {
...
export type MinimalDataModel = DataModelFromSchemaDefinition<typeof minimalSchema>
...
export async function getUser(db: DatabaseReader<ConvexLuciaAuth.DataModel>, userId: string) {
	return await db
		.query('users')
...
export async function getKey(db: DatabaseReader<ConvexLuciaAuth.DataModel>, keyId: string) {
	return await db
		.query('auth_keys')
